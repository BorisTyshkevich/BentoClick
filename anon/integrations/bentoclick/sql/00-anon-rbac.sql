...
--
-- Params: ${DB} (dashboard DB, e.g. bentoclick), ${META_DB} (anond meta DB,
-- e.g. altinity), ${DATA_DB} (real data DB, e.g. claude_otel), ${CLUSTER},
-- ${DICT_READER_PW} (a secret you supply; rotate like any DB credential).
...
CREATE USER IF NOT EXISTS anon_dict_reader
  ...
  DEFAULT ROLE NONE;
GRANT SELECT ON ${META_DB}.identifier_map TO anon_dict_reader
  ON CLUSTER '{cluster}';
...
REVOKE dictGet ON ${META_DB}.token_to_real    FROM anon_author_role ON CLUSTER '{cluster}';
...
REVOKE dictGet ON ${META_DB}.token_to_real    FROM ${DB}_anon_viewer_role ON CLUSTER '{cluster}';

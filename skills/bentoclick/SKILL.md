...
name: bentoclick
description: >
  Build, save, and share ClickHouse-backed dashboards — charts, tables, KPIs,
  and reports — including over anonymized/masked *_anon sandboxes. Use when the
  user asks to visualize ClickHouse data or build or save a dashboard, chart, or
  report.
---
...
> **Sandbox sources (`<db>_anon`):** if your data source is a tokenized masked
> sandbox, read **[sandboxes.md](sandboxes.md)** before authoring — discovery goes
> through `bentoclick.schema_guide`/`attr_guide` and the per-column token contract
> changes what you may filter, group, and show. Non-sandbox ClickHouse: author normally.
...
- **[sandboxes.md](sandboxes.md)** — tokenized masked sandboxes (`<db>_anon`):
  registry discovery via `bentoclick.schema_guide` / `attr_guide` and the
  per-column token contract (`real` / `identifier` / `redacted` / `attrmap`).
  Read it first when your data source is a `*_anon` database.
- **[panels/](panels/)** — one file per panel type (`bars`, `callouts`, `chart`,
  `combo`, `dataset`, `hero`, `html`, `kpi-strip`, `line`, `markdown`, `script`,
  `table`). Mandatory reading before you write a panel of that type (see the note
  at the top).
